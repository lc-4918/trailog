...
internal object MigrationSql {
    ...
    /** La mise de cote de la couche des points d'interet, eteinte : une base en place montrait ses
     *  points au lancement, et continue de le faire. */
    const val ADD_POI_MASKED = "ALTER TABLE settings ADD COLUMN poiMasked INTEGER NOT NULL DEFAULT 0"

    /** Les champs masques du tableau de bord : aucun, tous s'affichent (cf. DashboardField). */
    const val ADD_DASHBOARD_HIDDEN = "ALTER TABLE settings ADD COLUMN dashboardHidden TEXT NOT NULL DEFAULT ''"

    /** Le corps des compteurs du tableau de bord : seize points, la taille qu'ils avaient jusqu'ici. */
    const val ADD_DASHBOARD_FONT_SIZE =
        "ALTER TABLE settings ADD COLUMN dashboardFontSize INTEGER NOT NULL DEFAULT 16"

    /** Le corps des compteurs, desormais champ par champ : rien de regle, tous a leur taille par defaut. */
    const val ADD_DASHBOARD_FONT_SIZES =
        "ALTER TABLE settings ADD COLUMN dashboardFontSizes TEXT NOT NULL DEFAULT ''"

    /**
     * Les deux defauts corriges : la couleur de pente ne s'impose plus, le completement de l'altitude si.
     *
     * Une base en place les recoit AUSSI, et c'est voulu : ce ne sont pas des choix que l'utilisateur a
     * poses, ce sont ceux que l'application prenait pour lui, et ils etaient a l'envers - une aire
     * bariolee par defaut, et des traces sans altitude laissees sans profil.
     */
    ...
    /**
     * La coloration par pente de l'itineraire calcule, son trace et son profil, a part de celle des traces.
     * Elle suivait jusqu'ici le reglage du profil : la valeur en place se recopie dans les deux, pour que
     * rien ne change a l'ecran le jour de la mise a jour.
     */
    val ADD_ROUTE_SLOPE = listOf(
        "ALTER TABLE settings ADD COLUMN routeSlopeLine INTEGER NOT NULL DEFAULT 1",
        "ALTER TABLE settings ADD COLUMN routeSlopeProfile INTEGER NOT NULL DEFAULT 1",
        "UPDATE settings SET routeSlopeLine = profileSlope, routeSlopeProfile = profileSlope",
    )

    /** La trame des pentes (0,5 %), le trait des traces (4 dp, celui d'avant), les chevrons (eteints). */
    val ADD_TRACK_DISPLAY = listOf(
        "ALTER TABLE settings ADD COLUMN slopeClassTenths INTEGER NOT NULL DEFAULT 5",
        "ALTER TABLE settings ADD COLUMN trackLineWidth INTEGER NOT NULL DEFAULT 4",
        "ALTER TABLE settings ADD COLUMN trackDirectionArrows INTEGER NOT NULL DEFAULT 0",
    )

    /** Une couche coloriee par pente : aucune au depart. */
    const val ADD_LAYER_SLOPE_COLORED = "ALTER TABLE layers ADD COLUMN slopeColored INTEGER NOT NULL DEFAULT 0"
 ...
 * (cf. `ALL_MIGRATIONS`).
 */
internal const val DB_VERSION = 74
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // La mise de cote de la couche des points d'interet survit au redemarrage : une colonne, eteinte.
        ...
        // Le tableau de bord : les champs qu'on y masque, aucun au depart.
        ...
        // Le mode expert des reglages, eteint.
        ...
        // Les deux defauts remis a l'endroit (cf. MigrationSql.SET_NEW_DEFAULTS).
        ...
        // Le corps des compteurs du tableau de bord devient reglable, a sa valeur d'alors.
        private val MIGRATION_71_72 = object : Migration(71, 72) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(MigrationSql.ADD_DASHBOARD_FONT_SIZE)
            }
        }

        // Le corps des compteurs passe d'un reglage unique a un reglage par champ. Le precedent n'est pas
        // recopie : il n'a vecu qu'une soiree, dans un build de test, et jamais dans une version publiee.
        private val MIGRATION_72_73 = object : Migration(72, 73) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(MigrationSql.ADD_DASHBOARD_FONT_SIZES)
            }
        }

        // La coloration par pente de l'itineraire a part, l'affichage des traces, la pente par couche.
